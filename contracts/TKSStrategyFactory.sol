...
contract TKSStrategyFactory is Configable {
    address public dgas;
    ...
    function createStrategy(address _collateralToken, address _poolAddress, uint) onlyDeveloper external returns (address _strategy) {
        ...
        bytes memory bytecode = type(TKSStrategy).creationCode;
        ...
        TKSStrategy(_strategy).initialize(dgas, _collateralToken, _poolAddress, demaxPool, _collateralToken);
